...
export function CompressionMiddleware(req, res, next) {
    ...
    res.writeHead = createWriteHead(res.writeHead, function onResponseHeaders() {
        ...
        const method = req.get("Accept-Encoding").includes("gzip") ? "gzip" : "identity";
...
let toBuffer = (chunk, encoding) =>
    !Buffer.isBuffer(chunk) ? Buffer.from(chunk, encoding) : chunk;
