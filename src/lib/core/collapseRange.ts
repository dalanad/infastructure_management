export function collapseRange(pagesCount: number, current: number, max: number) {
	...
	if (hasStartEllipsis && !hasEndEllipsis) {
		...
		return [pages[0], -1, ...pages.slice(total - pageCount)];
	}
	if (!hasStartEllipsis && hasEndEllipsis) {
		const pageCount = max - 2;
		return [...pages.slice(0, pageCount), -1, pages[total - 1]];
	}
