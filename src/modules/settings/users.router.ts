...
route.get("/",
    ...
    async (req, res) => {
        ...
        let [items, count] = await req.orm.em.findAndCount(AuthUser, {}, {
            limit: size,
            offset: page * size,
            orderBy: { uid: "asc" }
        });
    ...
    validate({
            ...
            onError: async (e, req, res) => {
                ...
                res.locals.flash_msgs.push({
                    level: "danger",
                    msg: `<b>Validation Error Occured </b><br/><ul class="mt-2 mb-0"> ${ e.details.map(er => "<li>" + er.message + "</li>").join("") }</li></ul>`
                })
